...
func (cm *certManager) addFields(existing *cluster.DNSCredentials) []field {
	...
	return []field{
		{Key: "root_domain", Label: "Root domain", Def: rootDef, Note: "The domain you manage on the DNS provider — the TXT records for the cert challenge get written here. Usually the bare root, e.g. example.com.\nA subdomain like sub.example.com also works, as long as it shows up as its own zone in your Cloudflare / Aliyun console."},
