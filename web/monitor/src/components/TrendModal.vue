...
import { fetchTrafficTrend } from "../api";
import { formatBytes } from "../utils";
import type { SourceSummary, HourlyPoint } from "../types";
...
function buildOption(points: HourlyPoint[]): any {
  const data = granularity.value === "daily" ? aggregateDaily(points) : points;
  ...
  return {
    ...
    tooltip: {
      ...
      formatter(params: any) {
        ...
        const timeStr = granularity.value === "daily"
    ...
    grid: { left: 75, right: 30, top: 30, bottom: 100 },
    xAxis: {
      type: "time",
...
onMounted(async () => {
  try {
    trend.value = await fetchTrafficTrend(props.source.name);
  } catch {
    trend.value = [];
  }
  loading.value = false;
